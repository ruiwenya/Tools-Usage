...
# 用于base32隐写 根据base64隐写改编而来
s='''JVXXEYLMNF2HSIDNMF4SAY3PNZZWS43UEBZW63DFNR4SA2LOEB2GQZJAMNXXK4TBM5SSA33GEBWWC23JNZTSAYJAMNUG62LDMUXAU===
...
def get_base32_diff_value(s1, s2):
    base32chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ234567'
    res = 0
    for i in range(0,len(s2)):
        if s1[i] != s2[i]:
            return abs(base32chars.index(s1[i]) - base32chars.index(s2[i]))
    return res
...
for l in s.split('\n'):
   ...
   # b=(-(len(l.replace('=',''))%8))
    ...
    c=base64.b32encode(b).decode('utf-8')
    print(a,c)
    diff=(get_base32_diff_value(a,c))
    print(diff)
   
    pads_num = a.count('=')
    if diff:
        if(pads_num==4):
            bin_str += bin(diff)[2:].zfill(4) #16 20
            # bin_str += bin(diff)[2:].zfill(4)+'|' #16 20
        elif(pads_num==1):
            bin_str += bin(diff)[2:].zfill(3) #32 35
            # bin_str += bin(diff)[2:].zfill(3)+'|' #32 35
        elif(pads_num==6):
            bin_str += bin(diff)[2:].zfill(2) #8 10
            # bin_str += bin(diff)[2:].zfill(2)+'|' #8 10
        elif(pads_num==3):
            bin_str += bin(diff)[2:].zfill(1) #24 25
            # bin_str += bin(diff)[2:].zfill(1)+'|'#24 25s
